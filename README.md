...
- `AZURE_STORAGE_CONNECTION_STRING`（または既存の `STATIONS_CONNECTION_STRING` / `AzureWebJobsStorage`）  
  `master/stations.json` と `data/tide/{year}/{station}.json` を読むための接続文字列。

### 潮汐データの更新手順（TXT → JSON → Blob）
1. 気象庁の固定長テキストを `data_raw/jma/` 配下に配置（例: `data_raw/jma/2026_CS.txt`）。  
...
- `scripts/convertTideTxtToJson.ts` は CLI 引数で in/out/year/station_code を受け取り、日付整合性や hourly 要素数をチェック。異常時はエラー終了。
- `GetTides.ts` は `master/stations.json` で station_code を検証し、`data/tide/{year}/{station}.json` から該当日だけ返却する（なければ 404）。
